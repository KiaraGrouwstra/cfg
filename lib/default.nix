...
in {
  ...
  dryFlakes = pkgs: inputs: deps:
    listToAttrs (flatten (lists.map (dep:
      if strings.typeOf dep == "string" then {
        name = dep;
        value = let packages = inputs."${dep}".packages.${pkgs.system};
        in if hasAttr dep packages then packages."${dep}" else packages.default;
      } else
        mapAttrs (k: v: {
          name = v;
  ...
  prioritizeList = let
    prioritize = lower: higher:
      foldl' (o: k:
        o // {
          "${k}" = let v = getAttr k lower;
          in if hasAttr k o then (getAttr k o) ++ v else v;
  ...
  homeFolder = (baseDir:
    let
      makePath =
        (breadcrumbs: baseDir + "/${strings.concatStringsSep "/" breadcrumbs}");
      ...
      iterDir = (breadcrumbs:
        let
          ...
          processItem = (location: type:
            let breadcrumbs' = breadcrumbs ++ [ location ];
            in if type == "regular" then
              ...
              [ ]);
        in attrsets.mapAttrsToList processItem fileSet);
    in attrsets.mergeAttrsList (lists.flatten (iterDir [ ])));
